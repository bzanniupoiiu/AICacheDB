...
| 场景 | 测试对象 | 数据量 | 结果 |
| ---- | ---- | ---- | ---- |
| MTU 9000 + CPU 打满 | sendfile | 1 GB | **300.24 MB/s** |
| MTU 9000 + CPU 打满 | 优化后 RDMA Write | 1 GB | **339.73 MB/s** |
...
| 普通 TCP 代理双写 | 465.64 | 40.415 ms | 复制进入请求链路 |
| eBPF/XDP 旁路转发 | 450.29 | 41.695 ms | 虚拟机环境存在 XDP/用户态转发线程调度开销 |

结论：虚拟机下 eBPF/XDP 未体现吞吐优势，但它把复制从业务请求链路中拆出，适合做主从增量同步的旁路数据面。生产级优化方向是补强 TCP 流重组、批量转发、背压控制和丢包重放；在高性能网卡与真实内核数据路径下，XDP 的架构收益更明显。
